...
class AutonomyManager:
    ...
    def evaluate(self, telemetry: dict, constraint_result: dict = None,
                 upcoming_tasks: list = None, predictions: dict = None) -> dict:
        """
        Evaluate autonomy state from telemetry, constraints, tasks, and predictions.

        Parameters:
            telemetry: Current telemetry snapshot
            constraint_result: Output from constraint engine (risk_score, violations, margins)
        ...
        if not self.override_active:
            self._transition_mode(
                self.risk_score, recommended_mode, now
            )
