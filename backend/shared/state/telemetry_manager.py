...
def build_telemetry_frame(state: dict, fdir_alerts: list = None, source: str = "LIVE",
                          ai_result: dict = None, autonomy_snapshot: dict = None,
                          replan_summary: dict = None) -> dict:
    """
    Build telemetry frame from Mission State dict for WebSocket broadcast.

    source: "LIVE" (during ground contact), "PREDICTED" (during blackout),
            or "BUFFERED" (replay from onboard buffer dump).
    ai_result: optional AI monitor output for the same tick.
    autonomy_snapshot: optional autonomy status snapshot — used to surface
                      mode, objective, and combined risk on the frame.
    replan_summary: optional dict from CommandEngine.replan_on_autonomy_change.
                    When triggered, the frontend renders the loop-closure cue.
    """
    ...
    # Unified-loop additions (Phase 1): AI monitor, autonomy snapshot,
    # re-plan marker. All optional so legacy callers keep working.
    frame["ai_monitor"] = ai_result or {
        "anomaly_score": 0.0,
        "flagged_subsystems": [],
    ...
    if autonomy_snapshot:
        frame["autonomy_mode"] = autonomy_snapshot.get("mode")
        frame["autonomy_objective"] = autonomy_snapshot.get("current_objective")
        frame["combined_risk_score"] = autonomy_snapshot.get("combined_risk_score", 0.0)
        frame["risk_score"] = autonomy_snapshot.get("risk_score", 0.0)
    else:
        frame["autonomy_mode"] = None
        frame["autonomy_objective"] = None
        frame["combined_risk_score"] = 0.0
        frame["risk_score"] = 0.0

    if replan_summary and replan_summary.get("triggered"):
        frame["replan"] = {
            "triggered": True,
            "ai_driven": bool(replan_summary.get("ai_driven", False)),
            "mode_transition": replan_summary.get("mode_transition"),
            "objective_transition": replan_summary.get("objective_transition"),
            "affected_sequences": replan_summary.get("affected_sequences", []),
            "timestamp": replan_summary.get("timestamp"),
        }
    else:
        frame["replan"] = {"triggered": False}
